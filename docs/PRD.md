...
1. NICE에서 내보낸 XLS 5종을 로컬 웹 UI로 업로드·검토
...
| ID | 버그 | 해결 |
|----|------|------|
| B-001 | NICE XLS 헤더 오감지 ("1학년 3반 교과학습발달상황" 복합 셀을 헤더로 인식) | 길이 조건 `len(c) <= len(n)*2+2` 추가 |
| B-002 | NICE XLS 데이터 행에 "반" 컬럼이 없어 모든 학생 스킵 | 상단 셀에서 `_extract_class_info`로 추출 후 fallback |
...
- NICE 학생부 항목별 조회 XLS 포맷 (.xls, .xlsx)
